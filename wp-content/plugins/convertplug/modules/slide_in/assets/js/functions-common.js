...
jQuery(document).on('smile_customizer_field_change',function(e, single_data){
	...
	if( 'slidein_bg_image_size' in single_data ) {
		cp_update_ajax_bg_image_size( smile_global_data, '.cp-slidein-body', "", "slidein_bg_image", "opt_bg" );
	}
...
function global_initial_call( data ) {
	...
	cp_update_bg_image(data, ".cp-slidein-body", "", 'slidein_bg_image' );
...
parent.jQuery(window.parent.document).on('cp-image-change', function( e, name, url, val) {
	...
	if( name == 'slidein_bg_image' && name != 'undefined' && name != null ) {
