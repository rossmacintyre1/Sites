...
parent.jQuery(window.parent.document).on('smile-colorpicker-change', function( e, el, val) {
	...
	if(jQuery(el).hasClass('bg_color')){
		...
		gradiant_background(smile_global_data, ".cp-info-bar-body-overlay", "bg_color", "bg_gradient" );
	}
...
function global_initial_call( data ) {
	...
	//background setup
	gradiant_background(data,  ".cp-info-bar-body-overlay", "bg_color", "bg_gradient" );

	//custom css
	cp_add_custom_css( data );

	//	Modal Background Image
	cp_update_bg_image( data , ".cp-info-bar-body", "", "infobar_bg_image" );
...
jQuery(document).on('smile_customizer_field_change',function(e, single_data){
	...
	if( 'infobar_image_size' in single_data ) {
		cp_update_ajax_infobar_image_src( smile_global_data );
	}

	/**
	 * Infobar Background Image
	 */
	//	AJAX update image size - full / thumbnail / medium etc.
	if( 'infobar_bg_image_size' in single_data ) {
		cp_update_ajax_bg_image_size( smile_global_data, '.cp-info-bar-body', "", "infobar_bg_image", "opt_bg" );
	}
	...
	//close image settings
	if( 'close_info_bar' in single_data || 'close_txt' in single_data ) {
		cp_info_bar_close_img_settings(smile_global_data);
	}
 ...
 */
function cp_update_ajax_infobar_bg_image_size( smile_global_data ) {
	var cp_info_bar_body 		= jQuery(".cp-info-bar-body"),
		infobar_bg_image 	 	= smile_global_data.infobar_bg_image,
		infobar_bg_image_size 	= smile_global_data.infobar_bg_image_size,
		...
		if( infobar_bg_image !== "" ) {
		var img_data = {action:'cp_get_image',img_id:infobar_bg_image,size:infobar_bg_image_size};
		jQuery.ajax({
			url: smile_ajax.url,
			data: img_data,
			type: "POST",
			success: function(img){
				...
				jQuery('.smile-input[name="infobar_bg_image"]', window.parent.document ).attr('data-css-image-url', img );

				//	Set Background Image - Position, Repeat & Size
				image_positions( smile_global_data, ".cp-info-bar-body", "", "infobar_bg_image" );
			}
		});
	}
...
parent.jQuery(window.parent.document).on('cp-image-remove', function( e, url) {
	...
	jQuery('.smile-input[name="infobar_bg_image"]', window.parent.document ).attr('data-css-image-url', '' );
});
...
parent.jQuery(window.parent.document).on('cp-image-change', function( e, name, url, val) {
	...
	if( name == 'infobar_bg_image' && name != 'undefined' && name != null ) {
		cp_change_bg_img( smile_global_data, '.cp-info-bar-body' , '' , name, 'opt_bg', url, val );
	}

	//	Infobar - Image
	// Process for modal image - for variable 'modal_image'
	if( name == 'infobar_image' && name != 'undefined' && name != null ) {

		var infobar_image 	    		= smile_global_data.infobar_image,
			infobar_image_size		= smile_global_data.infobar_image_size,
		...
		if( infobar_image_size != "full" ) {
			//	Update image - ID|SIZE
			smile_global_data.infobar_image = val;
			cp_update_ajax_infobar_image_src( smile_global_data );
		} else {
...
/**
 * Update Infobar Image URL by AJAX
 */
function cp_update_ajax_infobar_image_src( smile_global_data ) {
	var 	infobar_image_size 	= smile_global_data.infobar_image_size,
		infobar_image 		= smile_global_data.infobar_image,
	...
	if( infobar_image !== "" ) {
		var img_data = {
			action:'cp_get_image',
			img_id: infobar_image,
			size: infobar_image_size
		};
		jQuery.ajax({
			url: smile_ajax.url,
			data: img_data,
...
/**
 * hide infobar on click of toggle button
 */
