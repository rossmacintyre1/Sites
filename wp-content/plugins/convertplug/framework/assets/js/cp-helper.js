 ...
 * background image
 */
function cp_update_bg_image( data, sel1, sel2, option ) {

    var sel1_elem            = jQuery(sel1),
        sel2_elem            = jQuery(sel2),
        modal_bg_image  = data[option];
    var new_style_image = jQuery('.smile-input[name="'+option+'"]', window.parent.document ).attr('data-css-image-url') || '';
    ...
    if( new_style_image != '' ) {
        ...
        if( sel2 != '' ) {            
            ...
            if( modal_size == 'cp-modal-custom-size' ){
                ...
                sel1_elem.css('background-image', 'url('+new_style_image+')');
            } else {
                sel1_elem.css('background-image', '');
                sel2_elem.css( 'background', 'url('+new_style_image+')');
            }
        } else {
            sel1_elem.css('background-image', 'url('+new_style_image+')');
        }
